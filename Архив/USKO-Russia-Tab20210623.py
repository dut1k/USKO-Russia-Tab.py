...
from Kumite import KumiteMainWindow_Ui, Frame_Header
import resources
...
class Ui_Form0(object):
    def setupUi0(self, Form0):
        ...
        self.btn_select_file.setStyleSheet("font-family: Gotham-Light; font-size: 15px;")
        self.btn_select_file.setObjectName("btn_Kata_qual")

        self.btn_Kata_qual.setGeometry(QtCore.QRect(100, 350, 350, 50))
        self.btn_Kata_qual.setStyleSheet("font-family: Gotham-Light; color: grey; font-size: 35px; ")
        self.btn_Kata_qual.setObjectName("btn_Kata_qual")

        self.btn_Kata_final.setGeometry(QtCore.QRect(100, 410, 350, 50))
        self.btn_Kata_final.setStyleSheet("font-family: Gotham-Light; color: grey; font-size: 35px; ")
        self.btn_Kata_final.setObjectName("btn_Kata_final")

        self.btn_Kumite.setGeometry(QtCore.QRect(500, 350, 200, 50))
        self.btn_Kumite.setStyleSheet("font-family: Gotham-Light; color: grey; font-size: 35px; ")
...
class MenuWindow(QWidget, Ui_Form0):
    def __init__(self):
        ...
        # QComboBox:!editable, QComboBox::drop-down:editable {
        #      background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,
        #                                  stop: 0 #f2f2f2, stop: 0.4 #f2f2f2,
        #                                  stop: 0.5 #f2f2f2, stop: 1.0 #f2f2f2);
        #     font-family: Gotham-Light;
        #     font-size: 15px;
        # }
        #
        ...
        self.KumiteWindowsClose = KumiteWindowsClose()
        self.ui_kataQual = KataQual_MainWindow()  # Окно с КАТА по флажкам
        self.ui_kataFinal = KataMainWindow()  # Окно с КАТА финал (по балам)
        self.ui_kumite = KumiteMainWindow_Ui()  # Окно КУМИТЕ
        self.header_kumite = Frame_Header(1)
        ...
        self.header_kumite.btn_closeWin.clicked.connect(self.KumiteWindowsClose.show)
        self.KumiteWindowsClose.buttonBox.accepted.connect(self.ui_kumite.clearKumiteData)
        self.KumiteWindowsClose.buttonBox.accepted.connect(self.showMainMenu)
        ...
        self.ui_KataQualToKumite.buttonBox.accepted.connect(self.ui_kataQual.NewCategory)
        ...
        self.ui_kataQual.combo.activated[str].connect(self.choiseCompitition_kataQual)
        ...
        self.ui_kataQual.btn_showData.clicked.connect(self.ui_kataQual.setMatchTitle)
        ...
        self.ui_kataFinal.btn_showData.clicked.connect(self.ui_kataFinal.setMatchTitle)

        self.ui_kumite.btn_showData.clicked.connect(self.ui_kumite.setMatchTitle)

        self.header_kumite.combo.activated[str].connect(self.choiseCompitition_Kumite)

        self.ui_kumite.winnerRed.toggled.connect(self.ui_kumite.setWinnerRed)
        self.ui_kumite.winnerWhite.toggled.connect(self.ui_kumite.setWinnerWhite)
...
# конструктор для matchName ############################################
    ...
    # def calcTabKataSpotrsmans(self, x):
        ...
        # comboBox_name_red_1.setStyleSheet(self.combo_style_disable)
        # comboBox_name_white_1.setStyleSheet(self.combo_style_disable)
...
#         self.ui_kataQual.lineEdit_region_red_1.setText(name.split('a11')[1])
    ...
    def kata_matchName(self):
        ...
        try:
            if len(ui_kata.matchName1.text().split(', ')) > 1:
                kk = ui_kata.matchName1.text().split(', ')[0]
                ui_kata.matchName1.setText(kk + ', ' + self.matchName[list(self.matchName.keys())[self.x]])
            else:
    ...
    def showKataQualWin(self):
        ...
        if self.sportsmans_ages_dict == {}:
            ...
            self.ui_kataQual.label_matchName2.hide()
        else:
            self.ui_kataQual.frame_sportsmans.show()
            self.ui_kataQual.label_matchName2.show()
    ...
    def showKataFinalWin(self):
        ...
        if self.sportsmans_ages_dict == {}:
            ...
            self.ui_kataFinal.label_matchName2.hide()
        else:
            self.ui_kataFinal.frame_sportsmans.show()
            self.ui_kataFinal.label_matchName2.show()
    ...
    def showKumiteWin(self):
        ...
        self.ui_kumite.show()
        if self.sportsmans_ages_dict == {}:
            self.ui_kumite.label_matchName2.hide()
        else:
            self.ui_kumite.label_matchName2.show()
        self.close()
...
class KumiteWindowsClose(QWidget, dialogWindow_Ui):
    def __init__(self):
        ...
        self.label.setText("Закрыть окна \"<b>Кумите</b>\"?")
