...
class Ui_Form0(object):
    def setupUi0(self, Form0):
        ...
        self.combo.setGeometry(QtCore.QRect(60, -30, 350, 22))
...
class MenuWindow(QWidget, Ui_Form0):
    def __init__(self):
        ...
        self.ui_kataQual.MenuButton.clicked.connect(self.ui_kataQual.NewCategory)
        ...
        if self.file:
            try:
                ...
                for z in range(10, len(sheet1.columns) + 4):
                    k.append('a' + str(z))
                sheet = pd.read_excel(self.file, usecols=cols, names=k)
...
# Ката груп     ############################################
                team_kata = sheet.loc[[2]].where(sheet == 'груп').dropna(how='all').dropna(axis=1)
...
# Кумите личн   ############################################
                team_kumite = sheet.loc[[2]].where(sheet == 'ком').dropna(how='all').dropna(axis=1)
...
# М             ############################################
                male_label = sheet.loc[[6]].where(sheet == 'м').dropna(how='all').dropna(axis=1)
...
# Ж             ############################################
                female_label = sheet.loc[[6]].where(sheet == 'ж').dropna(how='all').dropna(axis=1)
...
# Кумите Ж личн ############################################
    ...
    def calcTabKataSpotrsmans(self):
        ...
        if sender.text() == "М":  # pushButton pressed:
            ...
            self.TempSet.clear()
            self.TempSet = set(self.individ_kata_dict) - set(self.female_label_dict)
            ...
            self.ui_kataQual.comboBox_age.activated[str].connect(self.calcTabKataSpotrsmansMale)

        if sender.text() == "Ж":  # pushButton pressed:
            self.ui_kataQual.clearKataData()
            self.TempSet.clear()
            self.TempSet = set(self.individ_kata_dict) - set(self.male_label_dict)
            self.ui_kataQual.comboBox_age.setEnabled(True)
            self.ui_kataQual.comboBox_age.clear()
            self.ui_kataQual.comboBox_age.setStyleSheet("background-color: white")
            self.ui_kataQual.age_1.setStyleSheet("color: black; font-family: Gotham-Bold; font-size: 10pt;")
            ...
            self.ui_kataQual.comboBox_name_white_1.clear()
            self.ui_kataQual.comboBox_age.addItems(sorted([self.sportsmans_ages_dict[x] for x in self.TempSet]))
            self.ui_kataQual.comboBox_age.activated[str].connect(self.calcTabKataSpotrsmansFemale)

    def calcTabKataSpotrsmansMale(self):
        ...
        x = list(self.sportsmans_ages_dict.keys()).index(list(a2.keys())[list(a2.values()).index(n)])
        print('x', x)
        self.ui_kataQual.name_red_1.setStyleSheet("color: black; font-family: Gotham-Bold; font-size: 10pt;")
        self.ui_kataQual.name_white_1.setStyleSheet("color: black; font-family: Gotham-Bold; font-size: 10pt;")
        self.ui_kataQual.comboBox_name_red_1.setStyleSheet("background-color: white")
        self.ui_kataQual.comboBox_name_white_1.setStyleSheet("background-color: white")
        self.ui_kataQual.comboBox_name_red_1.addItems(sorted(self.df_Single_list[x]))
        self.ui_kataQual.comboBox_name_white_1.addItems(sorted(self.df_Single_list[x]))
        self.ui_kataQual.comboBox_name_red_1.setEnabled(True)
        self.ui_kataQual.comboBox_name_white_1.setEnabled(True)
        print('self.df_Single_list[x] =', self.df_Single_list[x])
        if self.ui_kataQual.comboBox_name_red_1.activated[str]:
            print('11111')
        # self.ui_kataQual.comboBox_name_red_1.activated[str].connect(self.setLabelRed)
        # self.ui_kataQual.comboBox_name_white_1.activated[str].connect(self.setLabelRed)

    def calcTabKataSpotrsmansFemale(self):
        n = self.ui_kataQual.comboBox_age.currentText()
        a2 = {k: v for k, v in
    ...
    def setLabelRed(self):
        print('name_red_1 =', self.ui_kataQual.comboBox_name_red_1.currentText())
        print('name_white_1 =', self.ui_kataQual.comboBox_name_white_1.currentText())
        n = self.ui_kataQual.comboBox_name_red_1.currentText()
        # self.df_Single_list[x].get(key
