...
class MenuWindow(QWidget, Ui_Form0):
        ...
        if self.file:
            ...
            try:
                ...
                for z in range(10, len(self.sheet1.columns) + 4):
                    k.append('a' + str(z))
                self.sheet = pd.read_excel(self.file, usecols=cols, header=None, names=k)
                ...
                type_set = set(self.sheet.iloc[5].values.tolist()) | set(self.sheet.iloc[7].values.tolist())

                if 'личн' in type_set:                                      # ЛИЧНЫЕ СОРЕВНОВАНИЯ
                    self.individ_dict = self.calc_dict(5, 'личн')
                if 'личн' not in type_set:                                      # ЛИЧНЫЕ СОРЕВНОВАНИЯ
                    self.individ_dict = {}
                if 'КОМ' in type_set:                                      # ЛИЧНЫЕ СОРЕВНОВАНИЯ
                    self.individ_dict = self.calc_dict(5, 'КОМ')
                if 'КОМ' not in type_set:                                      # ЛИЧНЫЕ СОРЕВНОВАНИЯ
                    self.individ_dict = {}

                # # ЛИЧНЫЕ СОРЕВНОВАНИЯ     ############################################
                # individ = self.sheet.loc[[5]].where(self.sheet == 'личн').dropna(how='all').dropna(axis=1)
                # sss = []
                # for i in range(1, len(individ.columns.tolist()) + 1):
                #     sss.append('ЛИЧ')
                # self.individ_dict = dict(zip(individ.columns.tolist(), sss))
                # print('ЛИЧНЫЕ СОРЕВНОВАНИЯ     ', self.individ_dict)
                # print(2)

                # КОМАНДНЫЕ СОРЕВНОВАНИЯ     ############################################
                team = self.sheet.loc[[5]].where(self.sheet == 'КОМ').dropna(how='all').dropna(axis=1)
                sss = []
                for i in range(1, len(team.columns.tolist()) + 1):
                    sss.append('КОМ')
                self.team_dict = dict(zip(team.columns.tolist(), sss))
                print('КОМАНДНЫЕ СОРЕВНОВАНИЯ     ', self.team_dict)
                print(3)

                # КУМИТЕ     ############################################
                kumite = self.sheet.loc[[8]].where(self.sheet == 'кумите').dropna(how='all').dropna(axis=1)
                sss = []
                for i in range(1, len(kumite.columns.tolist()) + 1):
                    sss.append('КУМИТЕ')
                self.kumite_dict = dict(zip(kumite.columns.tolist(), sss))
                print('КУМИТЕ     ', self.kumite_dict)
                print(4)

                # КАТА     ############################################
                kata = self.sheet.loc[[8]].where(self.sheet == 'ката').dropna(how='all').dropna(axis=1)
                sss = []
                for i in range(1, len(kata.columns.tolist()) + 1):
                    sss.append('КАТА')
                self.kata_dict = dict(zip(kata.columns.tolist(), sss))
                print('КАТА     ', self.kata_dict)
                print(5)

                # М             ############################################
                # male_label = self.sheet.loc[[7]].where(self.sheet == 'м', self.sheet == 'юн-ры').dropna(how='all').dropna(axis=1)
                male_label = self.sheet.loc[[7]].where(self.sheet == 'м').dropna(how='all').dropna(axis=1)

                sss = []
                for i in range(1, len(male_label.columns.tolist()) + 1):
                    sss.append('Мужчины')
                self.male_label_dict1 = dict(zip(male_label.columns.tolist(), sss))
                print('М             ', self.male_label_dict1)
                print(6)

                # # Ж             ############################################
                # female_label = self.sheet.loc[[7]].where(self.sheet == 'ж', self.sheet == 'д', self.sheet == 'юн-ки'). \
                # female_label_temp1 = self.sheet.loc[[7]].where(self.sheet == 'д').dropna(how='all').dropna(axis=1)
                # sss = []
                # for i in range(1, len(female_label_temp1.columns.tolist()) + 1):
                #     sss.append('Женщины')
                # self.female_label_dict1 = dict(zip(female_label_temp1.columns.tolist(), sss))
                # print('Ж1             ', self.female_label_dict1)
                # # Ж             ############################################
                # # female_label = self.sheet.loc[[7]].where(self.sheet == 'ж', self.sheet == 'д', self.sheet == 'юн-ки'). \
                # female_label_temp2 = self.sheet.loc[[7]].where(self.sheet == 'д').dropna(how='all').dropna(axis=1)
                # sss = []
                # for i in range(1, len(female_label_temp2.columns.tolist()) + 1):
                #     sss.append('Женщины')
                # self.female_label_dict2 = dict(zip(female_label_temp2.columns.tolist(), sss))
                # print('Ж2             ', self.female_label_dict2)
                # # Ж             ############################################
                # # female_label = self.sheet.loc[[7]].where(self.sheet == 'ж', self.sheet == 'д', self.sheet == 'юн-ки'). \
                # female_label_temp3 = self.sheet.loc[[7]].where(self.sheet == 'юн-ки').dropna(how='all').dropna(axis=1)
                # sss = []
                # for i in range(1, len(female_label_temp3.columns.tolist()) + 1):
                #     sss.append('Женщины')
                # self.female_label_dict3 = dict(zip(female_label_temp3.columns.tolist(), sss))
                # print('Ж3             ', self.female_label_dict3)
                # print(777)

                # ВСЕ ВОЗРАСТА  ############################################
                self.sportsmans_ages_dict = self.sheet.iloc[6].to_dict()
                list(map(self.sportsmans_ages_dict.__delitem__,
                         filter(self.sportsmans_ages_dict.__contains__, ('a10', 'a11', 'a12', 'a13'))))
                print('ВСЕ ВОЗРАСТА  ', self.sportsmans_ages_dict)
                print(8)
                ...
                print('М ВСЕ ВОЗРАСТА  ', self.male_sportsmans_ages_dict)
                print(9)
                ...
                print('Ж ВСЕ ВОЗРАСТА  ', self.female_sportsmans_ages_dict)
                print(10)
                ...
                print('КАТА М личн   ', self.individ_kata_male_dict)
                print(11)
                ...
                print('КАТА Ж личн   ', self.individ_kata_female_dict)
                print(12)
                ...
                print('КУМИТЕ М личн ', self.individ_kumite_male_dict)
                print(13)
                ...
                print('КУМИТЕ Ж личн ', self.individ_kumite_female_dict)
                print(14)
                ...
                print('КАТА груп     ', self.team_kata)
                print(15)
                ...
                print('КУМИТЕ ком   ', self.team_kumite)
                print(16)
                ...
                print('конструктор для matchName ', self.matchName)
                print(17)
    ...
    def calc_dict(self, rowNum, colName):
        ...
        print('ЛИЧНЫЕ СОРЕВНОВАНИЯ     ', self.colNum_dict)
        print(2)
